...
**Fase en curso:** Fase 6 (CI/CD) — workflow completo escrito y corregido, pendiente de pasos manuales en GitHub para dispararlo de verdad
**Último hito completado:** **`.github/workflows/ci-cd.yml` completo**, con 4 jobs encadenados (`ci` → `build` → `deploy-staging` → `deploy-production`). Incluye la "matriz + build + WIF + promote" que pide el árbol del enunciado, más una pieza que se explicó y decidió agregar a petición del usuario: **firma keyless de la imagen con Cosign** (reusa el mismo token OIDC de GitHub Actions, sin ninguna clave privada nueva — responde "¿quién produjo esta imagen?", que el digest SHA-256 por sí solo no garantiza). `deploy-production` tiene un gate manual real (`environment: production`, requiere aprobación humana en GitHub antes de correr). Se encontraron y corrigieron 2 bugs reales antes de intentar ejecutarlo: un output inexistente de `docker/build-push-action@v6` (`full_image` no existe, se compone a mano) y un `ARG NODE_BASE` que faltaba en el Dockerfile (la "matriz" de Node no habría probado nada distinto en realidad) — ambos verificados con builds locales reales.
**Siguiente paso concreto:** completar los **pasos manuales en GitHub** (no automatizables desde esta sesión — requieren la UI de github.com): (1) crear las 6 Repository Variables con los valores reales ya extraídos (ver tabla en `BITACORA-COMANDOS.md` § 6.4), (2) crear el `environment: production` con "Required reviewers" activado, (3) `git push github main`, (4) `git tag v1.0.0 && git push github v1.0.0` para disparar el pipeline completo por primera vez, (5) verificar en la pestaña Actions que los 4 jobs corren correctamente y que `deploy-production` queda pausado esperando aprobación.
...
> ⏸️ **Sesión pausada aquí el 2026-09-22 por la noche** — el usuario durmió muy poco la noche anterior, pidió parar para descansar. **Prioridad explícita para mañana: terminar TODO en una sola sesión**, porque el usuario tiene que grabar el video de explicación del trabajo justo después. El plan para retomar, en orden:
> 1. **Guiar paso a paso la configuración manual en GitHub** (sección 6.5 de `BITACORA-COMANDOS.md`): las 6 Repository Variables, el `environment: production` con reviewers, y disparar el pipeline con `git push github main` + el tag `v1.0.0`.
> 2. **Explicación pedagógica completa del pipeline, línea por línea** — el usuario pidió explícitamente entender bien cada línea de `.github/workflows/ci-cd.yml` porque tiene que explicarlo en el video (no basta con que funcione, necesita poder defenderlo hablado). Cubrir: qué es un pipeline de CI/CD en general, la diferencia entre los jobs `ci`/`build`/`deploy-staging`/`deploy-production`, por qué el `needs:` los encadena, qué hace cada acción de terceros usada (`checkout`, `google-github-actions/auth`, `docker/build-push-action`, `sigstore/cosign-installer`, `hadolint`, `gitleaks`), y por qué la seguridad real no depende del YAML sino del `attribute_condition` en GCP.
> 3. Verificar el resultado real del primer disparo del pipeline contra la API (mismo criterio de "no confiar solo en el log de GitHub" que se ha aplicado en todas las fases anteriores).
> 4. Fase 7 (bonus) si el tiempo alcanza — recordar que el usuario quiere intentar los 5 bonus, pero con la prioridad puesta en cerrar la base primero.
> 5. Fase 8 (documentación final: README con sección "Decisiones", `INFRA.md`, etc.) — mínimo indispensable si no da tiempo para todo.
> 6. **`terraform destroy` de ambos entornos al final** — decisión ya tomada por el usuario (evitar seguir gastando el crédito de $300/90 días una vez grabado el video). Hacerlo en orden inverso de dependencias, y confirmar con `gcloud` que no queda ningún recurso huérfano facturable tras el destroy (mismo rigor de verificación que en el resto del proyecto).
