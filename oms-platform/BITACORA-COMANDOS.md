...
### 8.1 · Redacción de los 6 documentos principales

A petición del usuario ("haz todo lo de la documentacion, adelanta todos esos archivos"), se escribieron de una sola vez los documentos pendientes de la Fase 8, basados en lectura directa y completa del código real — no plantillas genéricas ni contenido inventado:

- `README.md` (reescrito por completo — el esqueleto original del enunciado nunca se había rellenado): pre-requisitos con las versiones reales usadas, estructura real del árbol de archivos, cómo arrancar con los comandos reales, sección **"Decisiones"** obligatoria (política de uso de IA, sección 8 del enunciado) con 3 trade-offs reales y 3 cambios concretos verificables sobre el borrador de IA, y los 7 comandos de verificación del enunciado tal como se ejecutaron.
- `INFRA.md` (nuevo): arquitectura real desplegada, módulo por módulo (`network`, `database`, `compute`, `iam`), con los valores reales de cada `.tf` leído directamente, la tabla de diferencias legítimas staging/producción, y el resumen de recursos por módulo.
- `DEPLOYMENT.md` (nuevo): el flujo operativo completo — cómo logra idempotencia real el rol `oms_cloud_run` (comparación de 4 campos: imagen, cpu, memoria, max_instances, no solo la imagen), el detalle de `deploy.yml`/`rollback.yml`/`promote-canary.yml`, los 4 jobs de `ci-cd.yml` con su diagrama de dependencias, `canary-decision.yml`, y la evidencia del ciclo completo verificado en esta misma sesión (§ 6.21).
- `OBSERVABILIDAD.md` (nuevo): los 8 `database_flags` reales de Cloud SQL con el propósito documentado de cada uno, las 3 capas de healthcheck (Docker/Cloud Run probes/post-deploy de Ansible) y el hallazgo real de `/health` vs `/healthz`, y una sección explícita de qué NO está implementado (audit trail inmutable de REG-GDPR-003, degradación suave de Redis sin código de negocio real que probarla) — documentado como pendiente real, no ocultado.
- `CERTIFICADOS.md` (nuevo): TLS del Load Balancer (certificado managed de Google, estado real `PROVISIONING` por no tener dominio registrado, y la razón técnica del placeholder en minúsculas), el mecanismo completo de WIF/OIDC con sus 2 hallazgos reales ya documentados en el código (`allowed_audiences` incorrecto, `attribute_condition` incompleta), y Cosign keyless con el hallazgo de que la firma no viaja con `docker tag`+`push` (`cosign copy --only=sig`).
